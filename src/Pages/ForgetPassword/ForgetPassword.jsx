...
export default function ForgetPassword() {
    ...
    : {
        email: string().required('! Please enter your email').email('! Enter valid email')
    }
);
      ...
      {!isCodeSent ? (
        <>
          <div className="text-main-color font-bold flex items-center gap-1 mb-2">
            ...
            <label htmlFor="email" className="inline-block text-sm sm:text-[15px] lg:text-base">
              Enter your email
            </label>
          </div>
          ...
          {formik.errors.email && (
            <p className="font-bold text-sm text-red-800 pt-3">{formik.errors.email}</p>
          )}
        </>
      ) : (
        <>
          <div className="text-main-color font-bold flex items-center gap-1 mb-2">
            <MdOutlineEmail className="text-xl" />
            <label htmlFor="resetCode" className="inline-block text-sm sm:text-[15px] lg:text-base">
          ...
          {formik.errors.resetCode && (
            <p className="font-bold text-sm text-red-800 pt-3">{formik.errors.resetCode}</p>
          )}
        </>
      )}

      <div className="mt-4 md:flex justify-between items-center">
        <button className="btn-primary uppercase text-[13px] lg:text-[15px]" type="submit" disabled={formik.isSubmitting}>
